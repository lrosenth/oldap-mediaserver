...
- Open: Production rollout and matching internal service-secret provisioning remain operator-controlled. The untracked local `imageserver/test-images/` directory remains untouched.
- Risks/Assumptions: Startup reconciliation scans only the dedicated private upload root and never follows symlinks or removes registered generations. Cleanup may be delayed after accepted cancellation, but its reservation intentionally remains until physical deletion succeeds.

### Update 2026-08-23 18:08
- Decisions: Make mobile publication and compensation independent of Docker bind-mount boundaries; retain atomic visibility changes only within final media storage, preserve exact ownership evidence until deletion completes, and keep emergency rollback independent of an unprovisioned mobile-only secret.
- Implementation: Final-check corrections copy and fsync complete assets into a hidden final-filesystem staging directory before same-mount `RENAME_NOREPLACE`, install its ownership marker atomically with interrupted-marker recovery, compensate through a same-mount hidden path with marker-last recoverable deletion, reserve the doubled publication peak, reject symlinked or canonically overlapping deployment roots, and exempt only explicit rollback from new mobile-secret validation. Added mount-boundary, interrupted marker/publication/compensation, foreign-staging, closed-layout, capacity, rollback, and canonical-storage regressions; bumped mediahelper to `0.2.5`. The mandatory second review and full 260-test repository plus cross-repository compatibility matrix pass.
- Open: Deployment, Vault access, secret provisioning, and rollout remain operator-owned. Step 12 owns the mobile client queue.
- Risks/Assumptions: Hidden publication and compensation paths remain unrouted and exact-owner-bound on final media storage. The capacity estimate is intentionally conservative when private and final storage use separate physical filesystems.

### Update 2026-08-23 17:31
- Decisions: Complete Step 11E as additive deployment configuration only; keep the shared host default disabled, opt the two reviewed hosts into their next explicit deployment, isolate mobile upload state and credentials, and retain deployment, Vault access, secret provisioning, and rollout as operator actions.
- Implementation: Added the exact bounded `/media/v1` Caddy route, persistent private upload mount, hardened dedicated worker profile, reviewed limits, fail-closed Ansible secret/root validation, worker lifecycle and public 401 probe, ignored local artifacts, strict deployment-template tests, and synchronized OpenAPI and operations documentation. The complete 254-test suite, focused Black, Poetry metadata, YAML/Jinja/Compose rendering, Caddy validation, cross-repository compatibility tests, and Fasnacht Capture verification pass.
- Open: A later authorized deployment must add the distinct secret to both service runtimes, configure the API-owned service identity, run Ansible syntax/preflight on an Ansible host, and perform the staged live smoke test. Step 12 owns the mobile client queue.
- Risks/Assumptions: No Vault or secret value was read or changed and no deployment occurred. Local oldap-api GraphDB-dependent tests remain blocked by the missing `rosenth` fixture; FasnachtsPage retains pre-existing repository-wide check/lint findings although its 124 tests and production build pass. Untracked `imageserver/test-images/` remains untouched.

### Update 2026-08-23 16:40
- Decisions: Fence upload-owned filesystem effects with both the durable lease and existing per-upload lock; compensate only the closed OLDAP domain-rejection set; expire only unambiguous private retryable work and retain post-publication ambiguity.
- Implementation: Final-check corrections added synchronous pre/post-effect lease renewal, stale-worker serialization, retryable Step-11C active-job migration, defensive queue filtering, progress-based expiry renewal, safe pre-publication failure expiry, explicit cancellation/reinitialization after completed compensation, completed-cancellation cleanup acknowledgement, and fail-closed unknown OLDAP response classification. Added concise repository agent instructions and focused migration, stale-lease, expiry, cancellation, and response-classification regressions. The full 250-test suite, formatting, compilation, metadata, local Docker build, module-import, and native HEIF-loader checks pass.
- Open: Step 11E must perform the previously documented container/deployment verification and activate routing, persistence, secrets, and the standalone worker.
- Risks/Assumptions: Post-publication ambiguous failures intentionally retain final files and private evidence until identical OLDAP reconciliation succeeds. Mobile image dimension/resource ceilings remain a future hardening decision because v1 currently freezes only original byte size; existing derivative failures remain non-committing and cleanup-safe.

### Update 2026-08-23 16:01
- Decisions: Keep mobile processing in a separate lease-driven worker, reuse only lower-level image/HEIF/derivative/capacity primitives from ZIP ingest, and preserve independent mobile job state. Publish complete owner-marked files before OLDAP mutation; on definitive rejection atomically withdraw them into private upload storage before recoverable deletion, while retaining ambiguous publications for identical receipt replay.
- Implementation: Completed Step 11D with registry schema v2 migration, server-derived storage paths, deterministic events, durable checksum/rendition/publication/OLDAP/compensation phases, two-job renewable leases, exact-owned no-replace asset publication, purpose-specific short-lived service JWT calls, atomic permanent receipt recording, reclaimable expiry/cleanup, and standalone worker packaging. Added crash, race, lost-response, rejection, rendition, checksum, capacity, cancellation, migration, token-purpose, and repeated-cleanup tests.
- Open: Step 11E must provision the distinct JWT secret and persistent storage, activate/configure the worker, add Caddy/Ansible routing and limits, and perform deployment smoke/rollout checks. `/media/v1` remains externally disabled.
- Risks/Assumptions: Production publication requires work and final media paths on the same filesystem and Linux `renameat2`; real libvips HEIC decoding and deployment recovery remain Step-11E acceptance gates. Legacy `/upload`, ZIP jobs, media/IIIF token consumers, Caddy, Ansible, and secrets are unchanged. Untracked `imageserver/test-images/` remains untouched.

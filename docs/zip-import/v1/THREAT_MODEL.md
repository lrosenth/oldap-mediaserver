...
| TM-11 | P0 | Exploit in libjpeg/libtiff/libpng/libvips/FFmpeg/Poppler/veraPDF or MIME detector | Minimal runtime packages/loaders; no ImageMagick fallback; fixed UID/GID 65532 with zero capabilities; closed environment; no client/claim/token argument; seccomp-denied network; job-only workspace; no shell; time/RAM/PID/CPU/output limits; patch cadence | PARSER group, no-secret child test, Linux UID/capability/seccomp/runtime checks | A kernel/container escape remains possible; parser dependencies still require prompt security updates |
